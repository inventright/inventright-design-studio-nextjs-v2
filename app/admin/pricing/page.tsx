      ...
      <div className="bg-white rounded-lg shadow-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
        <div className="p-6">
          ...
          <div className="space-y-4">
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">
                Parent Product (Add-on For)
              </label>
              <select
                value={formData.parentProductKey || ''}
                onChange={(e) => setFormData({ ...formData, parentProductKey: e.target.value || null })}
                className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
              >
                <option value="">None (Standalone Product)</option>
                {products
                  .filter(p => p.category === 'department' && p.id !== product?.id)
                  .map(p => (
              ...
              <p className="mt-1 text-xs text-gray-500">
                Select a department if this is an add-on product
              </p>
            </div>
