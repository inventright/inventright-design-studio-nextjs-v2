...
// WordPress authentication with support for both cookies and Authorization header
export async function getCurrentUser() {
  try {
    ...
    userData = cookieStore.get('user_data')?.value;

    // Fallback to Authorization header if cookies not available
    if (!authToken || !userData) {
      ...
      if (authHeader && authHeader.startsWith('Bearer ')) {
        ...
        // Get user data from header or localStorage (client will send it)
